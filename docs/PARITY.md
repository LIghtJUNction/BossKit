...
Automatic application submission, resume mutation/synchronization, bulk greeting, automated reply, background scheduling, recruiter recommendation greeting, CAPTCHA/risk-control bypasses, phone/SMS/browser-repair login, and any non-BOSS provider are intentionally absent. Recruiter candidate search is bounded, read-only, and CLI-only.

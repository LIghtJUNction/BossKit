...
| Zhipin chat inbox | `boss chat inbox <LOCAL_JOB_ID>...` | CLI-only browserless latest-text snapshot for 1–5 exact existing conversations; one session refresh and friend-list scan, bounded/truncation-marked safe summaries, no polling/message/resume/application, and no MCP surface |
